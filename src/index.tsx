...
type Bindings = {
  ...
  GOOGLE_OAUTH_ENABLED?: string;  // Feature flag: 'true' or 'false'
}
...
// ========== Google OAuth 기능 플래그 ==========
// 활성화: GOOGLE_OAUTH_ENABLED=true
// 비활성화: GOOGLE_OAUTH_ENABLED=false (기본값)

function isGoogleOAuthEnabled(c: any): boolean {
  const enabled = c.env.GOOGLE_OAUTH_ENABLED
...
app.get('/api/auth/google', async (c) => {
  // 1. 기능 플래그 체크
  if (!isGoogleOAuthEnabled(c)) {
    return c.html(`
      <html>
        <head>
          <title>Google Login Disabled</title>
          <style>
            body { font-family: system-ui; max-width: 600px; margin: 50px auto; padding: 20px; }
            .notice { background: #fef3c7; border: 2px solid #f59e0b; padding: 20px; border-radius: 8px; }
            .info { background: #e0f2fe; border: 2px solid #0284c7; padding: 15px; border-radius: 8px; margin-top: 20px; }
        ...
        <body>
          <div class="notice">
            <h1>⚠️ Google 로그인 비활성화됨</h1>
            <p>Google OAuth 기능이 현재 비활성화되어 있습니다.</p>
            <p>기존 로그인 시스템(4자리 PIN)을 사용해주세요.</p>
          </div>
          
          <div class="info">
            <h3>🔧 관리자용: 활성화 방법</h3>
            <p><strong>로컬 개발:</strong></p>
            <p><code>.dev.vars</code> 파일에 추가:</p>
            <pre style="background: #1f2937; color: #f9fafb; padding: 15px; border-radius: 8px; overflow-x: auto;">GOOGLE_OAUTH_ENABLED=true
GOOGLE_CLIENT_ID=your-client-id.apps.googleusercontent.com
GOOGLE_CLIENT_SECRET=your-secret
GOOGLE_REDIRECT_URI=http://localhost:8787/api/auth/google/callback</pre>
            
            <p><strong>프로덕션:</strong></p>
            <pre style="background: #1f2937; color: #f9fafb; padding: 15px; border-radius: 8px; overflow-x: auto;">npx wrangler secret put GOOGLE_OAUTH_ENABLED
# → 입력: true

npx wrangler secret put GOOGLE_CLIENT_ID
npx wrangler secret put GOOGLE_CLIENT_SECRET
npx wrangler secret put GOOGLE_REDIRECT_URI</pre>
          </div>
...
app.get('/api/auth/google/callback', async (c) => {
  ...
  if (!isGoogleOAuthEnabled(c)) {
    return c.html(`
      <html>
        ...
        <body>
          <h1>❌ Google OAuth Disabled</h1>
          <p>Google OAuth feature is currently disabled.</p>
          <a href="/">← Back to App</a>
        </body>
      </html>
    `)
  ...
  try {
    ...
    // 3. Check if user exists in database (by email OR google_id)
    let user = await DB.prepare(`
      SELECT id, username, name, email, google_id FROM users 
    ...
    if (!user) {
      ...
      const result = await DB.prepare(`
        INSERT INTO users (username, email, name, password_hash, google_id)
        VALUES (?, ?, ?, ?, ?)
      `).bind(
        username,
        googleUser.email,
        ...
        googleUser.id   // Store Google ID
      ).run()
      
      console.log(`[OAuth] Created new user: ${username} (Google ID: ${googleUser.id})`)
      ...
      const needsUpdate = !user.google_id || !user.email
      ...
      if (needsUpdate) {
        await DB.prepare(`
          ...
          WHERE id = ?
        `).bind(googleUser.email, googleUser.id, user.id).run()
        
        console.log(`[OAuth] Updated Google info for user: ${user.username}`)
    ...
    const userEmail = user.email ? user.email.replace(/'/g, "\\'" ) : ''
    const userName = user.name ? user.name.replace(/'/g, "\\'" ) : ''
          ...
          <script>
            (function() {
              try {
                ...
                console.error('[OAuth] Error storing token:', error);
                alert('로그인에 성공했지만 토큰 저장에 실패했습니다. 다시 시도해주세요.');
              }
            })();
          </script>
        </head>
