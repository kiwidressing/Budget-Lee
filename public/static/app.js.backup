...
function renderCalendar(calendarData) {
  ...
  ['일', '월', '화', '수', '목', '금', '토'].forEach(day => {
    html += `<div class="text-center font-bold text-gray-600 py-2">${day}</div>`;
  });
        ...
        ${state.fixedExpenses.map(expense => `
          <div class="bg-white p-6 rounded-lg shadow">
            <div class="flex justify-between items-start mb-3">
              <h4 class="text-lg font-bold">${expense.name}</h4>
              <button onclick="deleteFixedExpense(${expense.id})" class="text-red-500 hover:text-red-700">
                <i class="fas fa-trash"></i>
              </button>
            </div>
            <p class="text-2xl font-bold text-red-600 mb-3">${formatCurrency(expense.amount)}</p>
